...
\paragraph{The two arms change places, and where they do is computable.} On a
\emph{tight} scale limit the normal-theory band is the narrower of the two, by
2 to 22\% over the same eight cells; on the \emph{loose} pivot limit the
ordering reverses. When we first met these two readings we treated the second as
an error and retracted it. It was not an error. Consequence~2 of
Section~\ref{sec:sensitivity} predicts a crossing at
$\lambda^\star=r_0^{-1/(e_2-e_1)}$, and a limit tight enough puts the setting on
one side of it and a loose limit on the other.
