...
pair of constructions therefore changes places between survey settings. We
observe exactly this: against a loose scale limit the deconvolution band is
narrower, and against a tight one the normal-theory band is, in the same eight
cells and with the same data (Section~\ref{sec:simulation}). Before the
elasticity was isolated these two readings looked like a contradiction and one of
them was retracted as an error; they are the two sides of $\lambda^\star$.
